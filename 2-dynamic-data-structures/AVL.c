...
int AVL_Balance_level (struct Node * root)
{return (root == NULL) ? 0 : (height(root->left) - height(root->right));}

struct Node * AVL_Rotate(struct Node * old_root, char * way)
{
...
{
    //just some casual BST insertion
    if (root == NULL)
    ...
    int balance = AVL_Balance_level(root);
    ...
    if (balance > 1 && key > root->left->key)
    {
        root->left = AVL_Rotate(root->left, "left");
        return AVL_Rotate(root, "right");
    }
    ...
    if (balance < -1 && key < root->right->key)
    {
        root->right = AVL_Rotate(root->right, "right");
        return AVL_Rotate(root, "left");
    }
    return root;
}
...
struct Node * AVL_Delete (struct Node * root, int key)
{
    if (root == NULL) return NULL;
        ...
        if (key > root->key)
        {
            ...
            if (AVL_Balance_level(root) > 1)
            {
                if(AVL_Balance_level(root->left) >= 0)
                {
                    //Left-left case
                    root = AVL_Rotate(root, "right");
                }
                else
                {
                    //Left-right case
        ...
        else if (key < root->key)
        {
            ...
            if (AVL_Balance_level(root) < -1)
            {
                if(AVL_Balance_level(root->right) <= 0)
                {
                    //Right-right case
                    root = AVL_Rotate(root, "left");
                }
                else
                {
                    //Right-left case
...
int main(){
    ...
    for (unsigned int j = 0; j < sizeof(ns) / sizeof(*ns); j++){
        ...
        for (int k = 0; k < REPETITIONS; k++){
            ...
            for (int l = 0; l < n; l++)
                root = AVL_Insert(root, students[l].index);
            ...
            for (int l = 0; l < n; l++)
                root = AVL_Delete(root, students[l].index);
